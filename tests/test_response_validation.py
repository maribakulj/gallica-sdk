...
def test_alto_rejects_html_success_payload() -> None:
    response = _response(b"<html><body>error</body></html>", content_type="text/html")
...
def test_image_rejects_html_success_payload() -> None:
    response = _response(b"<!doctype html><html></html>", content_type="text/html")
    gallica = Gallica(transport=StaticTransport(response))  # type: ignore[arg-type]
...
def test_pagination_rejects_malformed_and_non_integer_counts() -> None:
    ...
    invalid = _response(b"<livre><structure><nbVueImages>many</nbVueImages></structure></livre>")
    non_integer = Gallica(transport=StaticTransport(invalid))  # type: ignore[arg-type]
...
def test_content_search_rejects_invalid_count_results() -> None:
    response = _response(b'<results countResults="not-a-number"/>')
    gallica = Gallica(transport=StaticTransport(response))  # type: ignore[arg-type]
...
def test_content_search_rejects_malformed_geometry_and_score() -> None:
    bad_geometry = (
        b'<results countResults="1"><item><altoid>'
        b'<altoidstring hpos="x" vpos="1" width="2" height="3">ID</altoidstring>'
        b"</altoid></item></results>"
    )
...
def test_iiif_info_rejects_invalid_json_and_non_object_payloads() -> None:
    invalid = _response(b"not-json", content_type="application/json")
    malformed = Gallica(transport=StaticTransport(invalid))  # type: ignore[arg-type]
    with pytest.raises(GallicaResponseError, match="not valid JSON"):
        malformed.document("bpt6k1").page(1).iiif_info()

    array = _response(b"[]", content_type="application/json")
    non_object = Gallica(transport=StaticTransport(array))  # type: ignore[arg-type]
