...
| `page_alto` | `Page.alto() -> bytes` | RequestDigitalElement E=ALTO | `bytes` | live-validated | view numbers are 1-based<br>ALTO requests use the throttled alto rate bucket |
...
| `page_image` | `Page.image(width=1000, fmt='jpg') -> bytes` | IIIF Image | `bytes` | live-validated | width must be >= 1<br>widths up to 1000 use the throttled iiif rate bucket<br>width > 1000 uses the HD rate bucket<br>1000px is the recommended default |
