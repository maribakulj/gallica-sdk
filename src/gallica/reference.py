...
SERVICES: tuple[ServiceSpec, ...] = (
    ...
    {"id": "categories", "name": "Gallica Categories", "kind": "search-refinement", "status": "environment-limited", "base_url": "https://gallica.bnf.fr/services/Categories", "documentation": ("https://api.bnf.fr/fr/api-gallica-de-recherche",), "notes": ("accepts an SRU/CQL query through the SRU parameter", "returns JSON category values with approximate howMany counts when machine access is allowed", "at most 20 values are returned per category and some categories are not exhaustive", "API category names do not always match their CQL refinement fields", "public cold runners and external fetches returned HTML/403 on 2026-09-06; the SDK rejects that response instead of treating it as Categories JSON")},
    ...
    {"id": "iiif-presentation", "name": "Gallica IIIF Presentation", "kind": "structure", "status": "environment-limited", "base_url": "https://gallica.bnf.fr/iiif/", "documentation": ("https://api.bnf.fr/fr/api-iiif-de-recuperation-des-images-de-gallica",), "notes": ("BnF technical documentation currently advertises IIIF Presentation version 2", "document manifests use /iiif/ark:/12148/{id}/manifest.json", "the SDK detects Presentation protocol version explicitly and preserves raw JSON", "public cold external access returned HTTP 403 on 2026-09-07; live evidence records that limitation instead of claiming operability")},
