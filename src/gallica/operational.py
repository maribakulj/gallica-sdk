...
_SEMANTICS: dict[str, OperationalSemantics] = {
    ...
    "document_text": {"source_media_type": "text/plain", "output_semantics": "Plain OCR text for the document representation.", "errors": ("Unrecoverable HTTP errors propagate after throttling and bounded retries.",)},
    "content_search": {"source_media_type": "application/xml", "output_semantics": "Typed ContentSearchResults with items, total count and raw_xml.", "errors": ("page and start_result must be positive when supplied.", "Unrecoverable HTTP or malformed XML failures propagate to the caller.")},
    "page_text": {"source_media_type": "text/plain", "output_semantics": "Plain OCR text for exactly one 1-based view.", "errors": ("Invalid page/view values are rejected; HTTP failures propagate after retries.",)},
    "page_alto": {"source_media_type": "application/xml", "output_semantics": "Raw ALTO XML bytes for one 1-based view; the SDK does not impose an incomplete ALTO object model.", "errors": ("Invalid view values are rejected; HTTP failures propagate after retries.",)},
    "page_iiif_info": {"source_media_type": "application/json", "output_semantics": "Decoded IIIF Image info.json mapping for one view.", "errors": ("HTTP failures or invalid JSON propagate rather than becoming an empty mapping.",)},
    "page_image": {"source_media_type": "image/*", "output_semantics": "Raw image bytes for one IIIF Image request using the requested width and format.", "errors": ("width below 1 raises ValueError.", "Unrecoverable HTTP errors propagate after the appropriate standard or HD rate bucket.")},
    ...
    "corpus_fetch": {"source_media_type": "multiple", "output_semantics": "CorpusReport plus explicit files and append-only manifest entries for attempts actually executed.", "errors": ("At least one artifact must be requested.", "ALTO/images require explicit positive views and image_width must be positive.", "Ordinary per-artifact failures are captured in CorpusReport and do not stop independent artifacts or later ARKs.", "System-level interruptions are not swallowed.")},
}
