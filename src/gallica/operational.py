...
_SEMANTICS: dict[str, OperationalSemantics] = {
    ...
    "categories": {"source_media_type": "application/json", "output_semantics": "Typed Categories aggregation for an SRU/CQL query. CategoryValue preserves the service category, clean refinement value, optional display label and approximate howMany count; raw_json preserves the upstream response.", "errors": ("Empty queries raise ValueError before the request.", "HTML, malformed JSON, non-array payloads or malformed category entries raise GallicaResponseError.", "Counts are not promoted to exact result totals because Gallica documents howMany as approximate.")},
    ...
    "document_iiif_manifest": {"source_media_type": "application/json", "output_semantics": "IIIFPresentationManifest with explicit Presentation version, identifier, contexts, canvas count and raw_json. Version 2 and version 3 structures are validated separately and are not normalized into one another.", "errors": ("HTML, malformed JSON or non-object payloads raise GallicaResponseError.", "Conflicting v2/v3 contexts or structures raise GallicaResponseError.", "Version-specific identifier, sequences/items and canvas structure are validated before a typed manifest is returned.", "Unrecoverable HTTP errors propagate after bounded retries.")},
    ...
    "page_iiif_info": {"source_media_type": "application/json", "output_semantics": "IIIFImageInfo with explicit Image API version, identifier, contexts, protocol, advertised profile URIs, positive dimensions and raw_json.", "errors": ("HTTP failures, HTML, invalid JSON or non-object payloads raise instead of producing a partial model.", "Non-positive dimensions, invalid protocol/profile structures or conflicting v2/v3 indicators raise GallicaResponseError.", "When a version is advertised, the corresponding v2 @id or v3 id is required.")},
