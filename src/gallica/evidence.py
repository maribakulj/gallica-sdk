...
EVIDENCE: tuple[EvidenceSpec, ...] = (
    {"id": "live.vertical_slice", "kind": "live-test", "status": "passing-in-ci", "target": "tests/test_live.py::test_public_gallica_vertical_slice", "description": "Public smoke validation for SRU, structured Pagination, Toc, OAIRecord, ALTO and IIIF image access.", **_LAST_LIVE_OBSERVATION},
    {"id": "live.document_access", "kind": "live-test", "status": "passing-in-ci", "target": "tests/test_live.py::test_public_gallica_phase1_document_access", "description": "Public validation for ContentSearch excerpts/geometry/pagination, dated Issues resolution and safe texteBrut behavior, including explicit anti-bot challenge detection when cold runners are blocked.", **_LAST_LIVE_OBSERVATION},
...
def build_evidence_attestation(*, commit: str, run_url: str, observed_at: str | None = None) -> EvidenceAttestation:
    if len(commit) != 40:
    ...
    records = tuple(EvidenceAttestationRecord(evidence_id=item["id"], outcome="passed", observed_at=timestamp, commit=commit, run_url=run_url, confidence=item.get("confidence", "high")) for item in EVIDENCE if item["kind"] == "live-test")
    return {"schema_version": "1.0", "generated_at": timestamp, "commit": commit, "run_url": run_url, "records": records}
...
def load_evidence_attestation(path: str | Path) -> EvidenceAttestation:
    ...
    for raw in records_raw:
        ...
        records.append({"evidence_id": str(raw["evidence_id"]), "outcome": str(raw["outcome"]), "observed_at": str(raw["observed_at"]), "commit": str(raw["commit"]), "run_url": str(raw["run_url"]), "confidence": str(raw["confidence"])})
    return {"schema_version": "1.0", "generated_at": str(payload["generated_at"]), "commit": str(payload["commit"]), "run_url": str(payload["run_url"]), "records": tuple(records)}


def evidence_freshness(*, attestation: EvidenceAttestation | None = None, as_of: date | None = None) -> tuple[EvidenceFreshness, ...]:
    today = as_of or datetime.now(UTC).date()
    attested = {record["evidence_id"]: record for record in (attestation["records"] if attestation is not None else ())}
